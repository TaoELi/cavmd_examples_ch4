...
E0=0e-4
traj=1
echo "running nonequilibrate NVE for name=$name, nmol=$nmol, E0=$E0, traj=$traj"
...
INPUTFOLDER=$HOMEFOLDER/noneq_"$amp"_running_input_gaussian_$E0
...
if [ ! -d $INPUTFOLDER ]; then
    ...
    cp $TOTALFOLDER/cavity_loss_gaussian/* $INPUTFOLDER
    cd $INPUTFOLDER
...
NVEXYZ=$HOMEFOLDER/noneqxyznve_gaussian_"$amp"
mkdir $NVEXYZ
OUTFOLDER=$HOMEFOLDER/noneqout_gaussian_"$amp"
mkdir $OUTFOLDER

NVEFOLDER=$HOMEFOLDER/noneqnve_gaussian_"$amp"_"$traj"
...
if [ ! -d $NVEFOLDER ]; then
    ...
    cd $NVEFOLDER
    sed -i "s/mesitylene-pimd.1/ch4."$name".E0_"$E0".traj_noneq_"$amp"_"$traj"/" input_traj.xml
    sed -i "s/mesitylene-pimd.1/ch4."$name".E0_"$E0".traj_noneq_"$amp"_"$traj"/" in.lmp
...
rm -rf /tmp/ipi_ch4."$name".E0_"$E0".traj_noneq_"$amp"_"$traj"
