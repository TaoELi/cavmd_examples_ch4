...
def plotone(
    ...
    if colors == None:
        if markers == None:
            ...
            for i in range(len(xs)):
                if labels == None:
                    line, = ax.plot(xs[i], ys[i]/10**yorder, marker=markers[i], lw=lws[i], markersize=markersize, mfc=mfc, alpha=1.0 - i*alphaspacing if alphaspacing else alpha)
                else:
                    line, = ax.plot(xs[i], ys[i]/10**yorder, marker=markers[i], lw=lws[i], label=labels[i], markersize=markersize, mfc=mfc, alpha=1.0 - i*alphaspacing if alphaspacing else alpha)
                lines.append(line)
    else:
        if markers == None:
            for i in range(len(xs)):
            ...
            for i in range(len(xs)):
                if labels == None:
                    line, = ax.plot(xs[i], ys[i]/10**yorder, colors[i], marker=markers[i], lw=lws[i], markersize=markersize, mfc=mfc, alpha=1.0 - i*alphaspacing if alphaspacing else alpha)
                else:
                    line, = ax.plot(xs[i], ys[i]/10**yorder, colors[i], marker=markers[i], lw=lws[i], label=labels[i], markersize=markersize, mfc=mfc, alpha=1.0 - i*alphaspacing if alphaspacing else alpha)
                lines.append(line)
    if yscientific == True:
    ...
    if colorMap is not None:
        colormap = colorMap
        colors = [colormap(i) for i in np.linspace(colorMap_startpoint, colorMap_endpoint,len(ax.lines))]
        for i,j in enumerate(ax.lines):
            j.set_color(colors[i])
    if showlegend:
